...
class IFUcube:
    """
    A class for applying Principal Component Analyses (PCA) Tomography in
    Integral Field Unit (IFU) spectroscopy data cubes. See a description
    of the method in Steiner et al. 2009 (https://arxiv.org/pdf/0901.2266).
    """
    def __init__(self, fname, scidata='SCI',
                 primary='PRIMARY', spectral_dimension=3,
                 emission_line_mask=None):
        """
    ...
    def run_pca(self, components='all', emission_line_mask=None,
                emission_line_treatment='interpolate', write_fits=False,
                out_ext='_PCA.fits', normalization=False,
                norm_win=[None, None]):
        """
        Runs the PCA Tomography algoritm in the IFUcube
        (see detail in Steiner et al. 2009, https://arxiv.org/pdf/0901.2266.


        Parameters
        --------
        components: integer or string
            How many components to be saved. The standard is 'all',
            but it can become too large for large datacubes (e.g. MUSE datacubes).

            The other option is to select the number (integer) of
            components to be saved, e.g. components = 100.

        emission_line_mask: list of array
            The emission lines to be masked. It has to be a Boolean array
            (False value for the emission lines) with the same size
            as the spectral dimension of the datacube.

        emission_line_treatment: string
            If a 'emission_line_mask' is passed, 'emission_line_treatment' designs the treatment of the emission lines,
            wheter they will interpolated by 'best-fitting' curve (emission_line_treatment = 'interpolate')
        ...
        if self.em_mask is not None:
            if np.shape(np.array(self.em_mask))[0] == self.new_data.shape[0]:
                ...
                raise RuntimeError("'emission_line_mask' {}\
                    needs to have the same shape as the spectral dimension\
                    of the IFUcube {}".format(self.new_data.shape[0],
                                              np.shape(np.array(self.em_mask))))
        ...
        if (components != 'all'):
            ...
            self.tomograms = np.zeros((n_comp,
                                       self.new_data.shape[1],
                                       self.new_data.shape[2]))
        ...
        if write_fits:
            hdu = fits.PrimaryHDU([], header=self.header_data)
    ...
    def clean_data(self, components, fit_spline=True, smoothing_factor=0.1, write_fits=False):
        """
        Removes choosen components of the datacube, it is particularlly useful
        in 'Instrumental Fingerprint' removal from IFU spectroscopy data cubes
        (see the details in Menezes et al. 2014, https://arxiv.org/abs/1401.7078).


        Parameters
        --------
        components: integer or list
            Which components to be remove. It can be one component
            (e.g. components = 5) or a list (e.g. components = [22,30,45])

        fit_spline: Boolean
            Wheter to fit or not a spline to the Eigenvector before removing it.
            The standart it True.
        ...
        out_ext: string
            The extension to the fits file to be saved. Only applies if 'write_fits = True'.
            See the documentation in scipy manual:
        ...
        if isinstance(components, list):
            for i in components:
                ...
                if fit_spline:
                    ...
                    plt.plot(self.wl, spline(self.wl), lab="spline fit")
                    plt.show()
                    eigen_cube = data * spline(self.wl)[:, np.newaxis, np.newaxis]
                    clean_data = clean_data - eigen_cube
                elif(~fit_spline):
    ...
    def plot_scree_test(self, components=15, log_scale=True, scree_line=0.1):
        """
        Plots the scree test for the PCA Tomography (see details in Steiner et al. 2009,
         https://academic.oup.com/mnras/article/395/1/64/1078341).
        ...
        if log_scale:
            ...
            plt.hlines(scree_line,
                       xmin=0, xmax=components + 1,
                       color='red',
                       linestyle='--',
                       linewidth=0.5,
                       label='Variance = ' + str(scree_line) + ' %')
        else:
            plt.hlines(scree_line,
                       xmin=0,
                       xmax=components + 1,
                       ...
                       linewidth=0.5,
                       label='Variance = ' + str(scree_line) + ' %')
            plt.hlines(0, xmin=0, xmax=components + 1,
                       color='black',
                       linewidth=0.5,
                       label='Variance = 0.0 %')
